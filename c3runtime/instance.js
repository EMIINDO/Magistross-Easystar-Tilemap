...
/***/ function (module, exports) { module.exports = function () { this.pointsToAvoid = {}; this.startX; this.callback; this.startY; this.endX; this.endY; this.nodeHash = {}; this.openList; }; },
/***/ function (module, exports) { module.exports = function (parent, x, y, costSoFar, simpleDistanceToTarget) { this.parent = parent; this.x = x; this.y = y; this.costSoFar = costSoFar; this.simpleDistanceToTarget = simpleDistanceToTarget; this.bestGuessDistance = function () { return this.costSoFar + this.simpleDistanceToTarget; }; }; },
...
/***/ function (module, exports, __webpack_require__) {
        ...
        Heap = function () {
            ...
            Heap.prototype.pop = function (x) { return heappop(this.nodes, this.cmp); };
...
// --- SDK V2 IMPLEMENTATION ---
{
    ...
    C3.Behaviors.EasystarTilemap.Instance = class EasystarTilemapInstance extends globalThis.ISDKBehaviorInstanceBase {
        ...
        _initRuntimeHook() {
            ...
            if (!sdkInst) return;
            const getMapH = sdkInst.GetMapHeight || sdkInst.getMapHeight;
            const getMapW = sdkInst.GetMapWidth || sdkInst.getMapWidth;

            if (typeof getMapH !== "function") return;

            this.baseSetTileAt = sdkInst.SetTileAt || sdkInst.setTileAt;
            const mapHeight = getMapH.call(sdkInst);
            const mapWidth = getMapW.call(sdkInst);
        ...
        _tick() {
            ...
            if (sdkInst && typeof getMapH === "function") {
                const mapHeight = getMapH.call(sdkInst);
                const mapWidth = getMapW.call(sdkInst);
                if (!currentGrid || currentGrid.length !== mapHeight || currentGrid[0].length !== mapWidth) {
                    const tilegrid = globalThis.erenf.createArray(mapHeight, mapWidth);
                    globalThis.erenf.fillGridFromTilemap(tilegrid, sdkInst);
                    this.easystarjs["setGrid"](tilegrid);
                }
        ...
        doPathingRequest(tag_, x_, y_, x2_, y2_) {
            ...
            const getMapH = sdkInst ? (sdkInst.GetMapHeight || sdkInst.getMapHeight) : null;
            const getMapW = sdkInst ? (sdkInst.GetMapWidth || sdkInst.getMapWidth) : null;

            if (!sdkInst || typeof getMapW !== "function") {
                notFoundFunction();
                return;
            }

            const mW = getMapW.call(sdkInst);
            const mH = getMapH.call(sdkInst);

            // Coordinate debug log so you can monitor the data flow
            console.log(`DEBUG: Searching Path [Tag: ${tag_}] From (${x_}, ${y_}) To (${x2_}, ${y2_}) on the map ${mW}x${mH}`);

            if (x_ < 0 || y_ < 0 || x2_ < 0 || y2_ < 0 ||
                x_ > mW - 1 || x2_ > mW - 1 ||
                y_ > mH - 1 || y2_ > mH - 1) {
                console.warn(`DEBUG: Coordinates out of bounds ${mW - 1}x${mH - 1}!`);
                notFoundFunction();
            } else {
                this.easystarjs["findPath"](x_, y_, x2_, y2_, (path) => {
                    if (path === null) notFoundFunction(); else foundFunction(path);
                });
            }
        }
