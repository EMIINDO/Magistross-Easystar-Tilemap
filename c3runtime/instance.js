...
/***/ function(module, exports, __webpack_require__) {
	...
	**/
	...
	EasyStar.js = function () {
		...
		// CUSTOM SECTION FOR C2 BEHAVIOR
		this.clearInstanceList = function(){
			instances.length = 0;
		};

		this.getAcceptableTiles = function(){
			return acceptableTiles;
		};

		this.getDiagonalsEnabled = function(){
			return diagonalsEnabled;
		};
		
		this.getCostMap = function(){
			return costMap;
		};
		
		this.setCostMap = function(map){
			costMap = map;
		};
		
		this.getPointsToAvoid = function(){
			return pointsToAvoid;
		};

		this.setPointsToAvoid = function(pts){
			pointsToAvoid = pts;
		};
		
		this.getPointsToCost = function(){
			return pointsToCost;
		};
		
		this.setPointsToCost = function(pts){
			pointsToCost = pts;
		};
		
		this.getAllowCornerCutting = function(){
			return allowCornerCutting;
		};
		
		this.getDirectionalConditions = function(){
			return directionalConditions;
		};
		
		this.setDirectionalConditions = function(conditions){
			directionalConditions = conditions;
		};
		
		this.getGrid = function(){
			return collisionGrid;
		};
		// END OF CUSTOM SECTION FOR C2 BEHAVIOR
	    ...
	    this.calculate = function () {
	        ...
	        for (iterationsSoFar = 0; iterationsSoFar < iterationsPerCalculation; iterationsSoFar++) {
	            ...
	            if (instance.endX === searchNode.x && instance.endY === searchNode.y) {
	                var path = [];
					//CUSTOM: Added cost attribute to keep a record of each node cost
...
/******/ ]);
{
	...
	C3.Behaviors.EasystarTilemap.Instance = class EasystarTilemapInstance extends C3.SDKBehaviorInstanceBase
	{
		...
		LoadFromJson(o)
		{
			...
			var tilegrid = globalThis.erenf.createArray(this.inst.GetSdkInstance().GetMapHeight(), this.inst.GetSdkInstance().GetMapWidth());
			...
			globalThis.erenf.fillGridFromTilemap(tilegrid, this.inst);
		...
		isTileWalkable(x, y, sourceX, sourceY) 
		{
			...
			// dop2000		
			var pointsToAvoid = this.easystarjs["getPointsToAvoid"]();
			//console.log("Points-" + JSON.stringify(pointsToAvoid));
			...
			if(sourceX && sourceY){
				...
				if (directionalCondition) {
					var direction = globalThis.erenf.calculateDirection(sourceX - x, sourceY - y);
					var directionIncluded = function () {
						for (var i = 0; i < directionalCondition.length; i++) {
							if (directionalCondition[i] === direction) return true;
						}
			...
			// dop2000
			if (pointsToAvoid[y] === undefined || pointsToAvoid[y][x] === undefined) {
